...
struct Vertex
{
	...
	static VkVertexInputBindingDescription getBindingDescription()
	{
		...
		bindingDescription.binding = 0;//缓冲区编号
...
private:
	...
	void createRenderPass()
	{
		...
		depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;//不存储深度数据（绘制完成后不使用，如果后处理要用需开启）
		...
		subpass.pDepthStencilAttachment = &depthAttachmentRef;//一个subpass只有一个深度/模板缓冲区

		VkSubpassDependency dependency{};
		dependency.srcSubpass = VK_SUBPASS_EXTERNAL;//上一个执行的subpass，（渲染通道之前或之后的隐式子通道）
		dependency.dstSubpass = 0;//当前subpass
		dependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;//需等待的操作发生的阶段
		dependency.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;//需等待的操作（0表示任意操作，我们不关心）
		dependency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;//当前subpass要执行的操作处于的阶段
		dependency.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;//当前subpas要执行的操作
	...
	void createDescriptorSetLayout()
	{
		...
		uboLayoutBinding.binding = 0;//对应shader中uniform的binding
		uboLayoutBinding.descriptorCount = 1;
		uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;//指定描述符类型
		uboLayoutBinding.pImmutableSamplers = nullptr;
		uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;//指定在哪个阶段引用描述符
	...
	void createGraphicsPipeline()
	{
		...
		depthStencil.depthTestEnable = VK_TRUE;//Ztest on
	...
	void createFramebuffers()
	{
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++)
		{
			...
			depthImageView//所有交换链图像共用一张深度图（有信号量，不会出现同步问题）
			};
	...
	void createDepthResources()
	{
		...
		//直接分配最佳内存，深度缓冲区gpu自己写自己用，不需要像顶点缓冲区一样从cpu传数据
		createImage( swapChainExtent.width, swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory );
		depthImageView = createImageView( depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT );
	}

	//给定一批图像格式和图像在内存中的布局方式，找出在该图像内存布局方式下至少支持给定features的图像格式
	VkFormat findSupportedFormat( const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features )
	{
		for (VkFormat format : candidates)
		{
			...
			//二进制位掩码，表示至少要支持features中的所有属性（可以额外支持其他属性）
	...
	void transitionImageLayout( VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout )
	{
		...
		//将暂存缓冲区数据copy到image内存之前（用于写入image的布局）
		if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
		{
			barrier.srcAccessMask = 0;//在barrier之前必须发生的操作
			barrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;//在进行该操作时等待barrier

			sourceStage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;//barrier之前操作发生的管线阶段
			destinationStage = VK_PIPELINE_STAGE_TRANSFER_BIT;//等待barrier的操作发生的管线阶段
		}
		//copy后（用于着色器读取image的布局）
	...
	void copyBufferToImage( VkBuffer buffer, VkImage image, uint32_t width, uint32_t height )
	{
		...
		region.bufferRowLength = 0;//指定像素在内存中的布局方式
		region.bufferImageHeight = 0;//指定像素在内存中的布局方式
		//指定将像素复制到图像的哪一部分
	...
	//把显存映射到内存（指针data），然后在CPU端将数据复制到data
	//在后台会将数据传输到 GPU 显存
	void createVertexBuffer()
	{
		...
		VkBuffer stagingBuffer;
		VkDeviceMemory stagingBufferMemory;
		//在显存上CPU可以访问的区域（通常速度较慢）创建缓冲区并分配内存
		createBuffer( bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory );

		void* data;
		//将显存上stagingBufferMemory映射到主存上data
	...
	void recordCommandBuffer( VkCommandBuffer commandBuffer, uint32_t imageIndex )//要写入的当前交换链图像的索引
	{
		...
		vkCmdBindVertexBuffers( commandBuffer, 0, 1, vertexBuffers, offsets );//第二个参数表示缓冲区编号binding
		vkCmdBindIndexBuffer( commandBuffer, indexBuffer, 0, VK_INDEX_TYPE_UINT32 );//we can only have one index buffer
		vkCmdBindDescriptorSets( commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr );
		vkCmdDrawIndexed( commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0 );//显示发出一个draw call,往framebuffer中对应的color image和depthimage进行写入
